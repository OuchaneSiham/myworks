...
function Profile()
{
  ...
  // console.log("DRAFT STATE:", updatedData);
    ...
    const handleDecline = async(reqId) =>{
      const getToken = localStorage.getItem("token");
      try{
        const data = await fetch(`${API_BASE_URL}/users/friends/request/${reqId}`,{
          headers: {"Authorization": "Bearer " + getToken},
          method: "delete",
        });
        if(data.ok)
    ...
    const handleSendRequest = async (targetId) =>{
      const getToken = localStorage.getItem("token");
      try{
        const resp = await fetch(`${API_BASE_URL}/users/friends/request/${targetId}`, {
          headers: {"Authorization": "Bearer " + getToken},
          method: "POST",
    ...
    const handleLogout = async () => {
      const getToken =  localStorage.getItem("token");
      try{
        const logg = await fetch(`${API_BASE_URL}/users/logout`, {
          headers: { "Authorization": "Bearer " + getToken },
          method: "POST",
        })
        if(logg.ok){
